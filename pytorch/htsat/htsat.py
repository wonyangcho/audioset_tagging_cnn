...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.enable_tscam:
            ...
            x = x.permute(0, 1, 3, 2, 4).contiguous().reshape(
                B, C, c_freq_bin, -1)
            ...
            if self.htsat_attn_heatmap:
                ...
                attn = attn.permute(0, 2, 1, 3).contiguous().reshape(
                    B, c_freq_bin, -1)
            ...
            if self.htsat_attn_heatmap:
                fpx = interpolate(torch.sigmoid(x).permute(
                    0, 2, 1).contiguous() * attn, 8 * self.patch_stride[1])
            else:
                fpx = interpolate(torch.sigmoid(x).permute(
            ...
            if self.loss_type == "clip_ce":
                ...
                # }
                #
                output_dict = {'clipwise_output': torch.sigmoid(x),
            ...
            fpx = x.permute(0, 2, 1).contiguous().reshape(
                B, C, frames_num // (2 ** (len(self.depths) + 1)), frames_num // (2 ** (len(self.depths) + 1)))
            B, C, F, T = fpx.shape
            c_freq_bin = F // self.freq_ratio
            fpx = fpx.reshape(B, C, F // c_freq_bin, c_freq_bin, T)
            fpx = fpx.permute(0, 1, 3, 2, 4).contiguous().reshape(
                B, C, c_freq_bin, -1)
            fpx = torch.sum(fpx, dim=2)
            fpx = interpolate(fpx.permute(0, 2, 1).contiguous(),
    ...
    def forward(self, input, mixup_lambda=None, infer_mode=False):
        ...
        if infer_mode:
            ...
            if self.training:
                cur_pos = random.randint(
                    0, (self.freq_ratio - 1) * self.spec_size - 1)
                ...
                clipwise_output = torch.zeros_like(
                    output_dicts[0]["clipwise_output"]).float().to(x.device)
                framewise_output = torch.zeros_like(
                    output_dicts[0]["framewise_output"]).float().to(x.device)
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                if self.training:
                    x = self.crop_wav(
                        x, crop_size=self.freq_ratio * self.spec_size)
                    x = self.reshape_wav2img(x)
                    output_dict = self.forward_features(x)
                else:
                    ...
                    for cur_pos in range(0, x.shape[2] - crop_size - 1, overlap_size):
                        tx = self.crop_wav(
                            x, crop_size=crop_size, spe_pos=cur_pos)
